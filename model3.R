...
########## Below code is just for getting files in R from github or gdrive ##########

####  file.edit('~/.Rprofile')
### inserting this   to get rif off SSH error --> options(rpubs.upload.method = "internal")

#library(RCurl)

###  https://drive.google.com/file/d/0B2YVITpwU9nPWm1pOGlMQU1ucGc/view?usp=sharing

#gdrive.Connection <- getForm("https://spreadsheets.google.com/spreadsheet/pub", 
#            hl ="en_US", key = "0B2YVITpwU9nPWm1pOGlMQU1ucGc", 
#           output = "csv", 
#          .opts = list(followlocation = TRUE, verbose = TRUE, ssl.verifypeer = FALSE)) 

#sp.dfpredFinal.All <- read.csv(textConnection(gdrive.Connection))

## gitHub.URL<- getURL("https://raw.github.com/jasmeetsasan2014/BA_Assignment/blob/master/CompleDataSetWithForecast.csv")

## sp.dfpredFinal.All<- read.csv(text=gitHub.URL)
